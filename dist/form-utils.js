...
angular.module('mwFormUtils.responseUtils', []).factory('mwFormResponseUtils', function mwFormResponseMergerFactory() {
    ...
    var questionTypesWithDefaultAnswer = ['text', 'textarea', 'number', 'date', 'time', 'email', 'range', 'url', 'star'];
    ...
    service.getResponseSheetRow = function (formData, responseData) {
        ...
        var questionsWithSpecialFormatting = ["radio", "checkbox", "select", "grid", "priority", "division"];
        ...
        for (var i = 0; i < questions.length; i++) {
            ...
            if (questionsWithSpecialFormatting.indexOf(question.type) !== -1) {
                if (question.type == 'radio' || question.type == 'select') {
                    ...
                    if (!response) {
                        if (question.grid.cellInputType == 'radio') {
                            question.grid.rows.forEach(function () {
                                result.push("");
                            });
                        } else {
                            question.grid.rows.forEach(function () {
                                question.grid.cols.forEach(function () {
                                    result.push("");
                                });
                            });
                        }
